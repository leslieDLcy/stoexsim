...
!DEBUG
     ...
     :                    alog10((pi/(2.0*981.0))*husid_end)  ! for geometric mean 
          ...
          if (write_site_files) then
            ...
            if (ihypo==1 .and. isim == 1) then
            
              facc = ' '
              facc = f_stem(1:nc_f_stem)//'_acc_s000.out'
              write(facc(nc_f_stem+7:nc_f_stem+9),'(i3.3)') isite
     ...
     :                 isim,isite,HypoDistance,d_cd2f, fpar)
              ...
              fhusid = ' '
              fhusid = f_stem(1:nc_f_stem)//'_husid_s000.out'
...
!        05/17/15 - Call q_func, which is appended to the bottom of this file.
     ...
     :                   (1.0/(1.0+(f/cornerF)**2.0))
      ...
      if (f /= 0.0) then
          ...
          gamma=w/(2.*Q*beta)
c this is a bug - we need to change to (see _be file):          gamma=w/(2.*Q*c_q)
      endif
